...
const unsigned int MAX_PWM_VALUE = 100; // Max PWM given 8 bit resolution
...
void set_motors_pwm(float left_pwm, float right_pwm) {
  ...
  if (left_pwm  >  (float)MAX_PWM_VALUE) left_pwm  =  (float)MAX_PWM_VALUE;
  if (left_pwm  < -1 * (float)MAX_PWM_VALUE) left_pwm  = -1 * (float)MAX_PWM_VALUE;
  if (isnan(right_pwm)) right_pwm = 0.0;
  if (right_pwm >  (float)MAX_PWM_VALUE) right_pwm =  (float)MAX_PWM_VALUE;
  if (right_pwm < -1 * (float)MAX_PWM_VALUE) right_pwm = -1 * (float)MAX_PWM_VALUE;
...
void cardiod(float t, float a, float& x, float& y) {
  x = a * cos(t) * (1 - cos(t));
  y = a * sin(t) * (1 - cos(t));
}
...
void loop() {
  ...
  // States used to calculate target velocity and heading
  float cardiod_a = 0.1; // Radius
  float cardiod_t_scale = 2.0; // speedup factor
  float x0, y0;
  cardiod(0.0, cardiod_a, x0, y0);
  float last_x, last_y;
  cardiod(-cardiod_t_scale * target_period_ms / 1000.0, cardiod_a, last_x, last_y);
  float last_dx = (x0 - last_x) / ((float)target_period_ms / 1000.0);
  float last_dy = (y0 - last_y) / ((float)target_period_ms / 1000.0);
  float last_target_v = sqrtf(last_dx * last_dx + last_dy * last_dy);
  float target_theta = 0.0; // This is an integrated quantity
  ...
  while (true) {
    ...
    // Calculate target forward velocity and target heading to track the cardiod trajectory
    // of 0.5 meter radius
    float x, y;
    cardiod(cardiod_t_scale * t, cardiod_a, x, y);
